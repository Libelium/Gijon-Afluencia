...
# starttls (587) o ssl (465). Los consumidores no envian en claro.
MAIL_ENCRYPTION="${MAIL_ENCRYPTION:-starttls}"
# Los consumidores solo envian avisos por correo con MAIL_ENABLED=true: se activa en cuanto
# hay servidor, para que rellenar MAIL_HOST baste.
if [ -n "$MAIL_HOST" ]; then MAIL_ENABLED=true; else MAIL_ENABLED=false; fi
...
VARS=(PROJECT_NAME ENVIRONMENT IMAGE_REGISTRY GATEWAY_NAMESPACE GATEWAY_PUBLIC
  ...
  MAIL_PORT MAIL_ENCRYPTION MAIL_ENABLED
