...
class CrowdFlowsMunicipalityAll(Job):
    def __init__(
        self,
    ...
    def __start_jobs(self, user_id, entities) -> None:
        ...
        """
        from tasks.crowd import flows_municipality_job
        ...
        while start_date < end_date:
            ...
            notification = CrowdFlowsMunicipalityRequest(
                user_id=user_id,
                entities=entities,
            ...
            logging.info(
                f"Delaying flows municipality job for user {user_id} - {start_date} - {next_date}"
            )

            flows_municipality_job.delay(notification)
    ...
    def handle(self) -> None:
        ...
        """
        logging.info(f"Starting crowd flows municipality for all users")
        with main_session(self._injected_main) as db:
            ...
            try:
                ...
                for user in users:
                    logging.info(f"Starting crowd flows municipality for user {user.id}")
